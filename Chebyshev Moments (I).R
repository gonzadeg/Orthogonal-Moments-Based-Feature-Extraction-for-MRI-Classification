...
# Calcular polinomios de Chebyshev (I)
tchebyshev_polynomial = function(n, x) {
...
# Calcular los momentos de Chebyshev (I)
calculate_chebyshev_moment = function(img_vector, p, q, nrow, ncol) {
  m = as.numeric(nrow)
  n = as.numeric(ncol)
  ...
  # Calcular polinomios
  Tp = tchebyshev_polynomial(p, x)
  Tq = tchebyshev_polynomial(q, y)
  
  # Reconstrucción de imagen
  img_matrix = matrix(img_vector, nrow = m, ncol = n)
  
  # Calcular el momento de Chebyshev
  CHpq = img_matrix * (Tp %*% t(Tq))
  moment = sum(CHpq) * (2 / m) * (2 / n)
  return(moment)
}
...
process_batch = function(image_paths, class_name, max_dim) {
  ...
  features = lapply(images_resized, function(img) {
    ...
    p_max            = 12
    q_max            = 12
    chebyshev_moments = matrix(0, nrow = p_max + 1, ncol = q_max + 1)
    
    for (p in 0:p_max) {
      for (q in 0:q_max) {
        chebyshev_moments[p + 1, q + 1] = calculate_chebyshev_moment(
    ...
    chebyshev_features        = as.vector(chebyshev_moments)
    names(chebyshev_features) = paste0("DCM_p", 0:p_max, "_q", 0:q_max)
    return(chebyshev_features)
  })
