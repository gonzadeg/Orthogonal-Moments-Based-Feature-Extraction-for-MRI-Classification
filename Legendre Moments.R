...
legendre_polynomial = function(n, x) {
...
# Calcular los momentos de Legendre Continuos (CLM)
calculate_legendre_moment = function(img_vector, p, q, nrow, ncol) {
  m = as.numeric(nrow)
  n = as.numeric(ncol)
  ...
  # Calcular polinomios
  Pp = legendre_polynomial(p, x)
  Pq = legendre_polynomial(q, y)
  
  # Reconstrucción de imagen
  img_matrix = matrix(img_vector, nrow = m, ncol = n)
  
  # Calcular el momento de Legendre
  Lpq    = img_matrix * (Pp %*% t(Pq))
  moment = sum(Lpq) * (2 / m) * (2 / n)
}
...
process_batch = function(image_paths, class_name, max_dim) {
  ...
  features = lapply(images_resized, function(img) {
    ...
    # Calcular los momentos de Legendre
    p_max            = 12
    q_max            = 12
    legendre_moments = matrix(0, nrow = p_max + 1, ncol = q_max + 1)
    
    for (p in 0:p_max) {
      for (q in 0:q_max) {
        legendre_moments[p + 1, q + 1] = calculate_legendre_moment(
    ...
    legendre_features        = as.vector(legendre_moments)
    names(legendre_features) = paste0("CLM_p", 0:p_max, "_q", 0:q_max)
    return(legendre_features)
  })
...
# Obtener cantidad total de imágenes procesadas
total_imagenes = nrow(train_data)

# Obtener cantidad de características por imagen (excluyendo la columna 'class')
cantidad_features = ncol(train_data) - 1

# Imprimir resultados
cat("Cantidad total de imágenes procesadas:", total_imagenes, "\n")
cat("Cantidad de características por imagen:", cantidad_features, "\n")
...
# 2. Calcular IC del 95% y estadísticas globales por feature (sin separar por clase)
